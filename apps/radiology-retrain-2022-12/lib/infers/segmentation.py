...
class Segmentation(InferTask):
    ...
    def post_transforms(self, data=None) -> Sequence[Callable]:
        ...
        predArray = data['pred'].get_array()
        lymphNodeArray = predArray[self.labels['lymphnodes']]
        print(predArray.shape)
        writer = ITKWriter()
        writer.set_data_array(lymphNodeArray, channel_dim=None)
        writer.set_metadata({'affine': data['pred'].affine})
        writer.write("/tmp/lymphnode-pred.nrrd")
